...
public class linspirer_fakeuploader extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btn_gettastics.setOnClickListener(view -> {
                    ...
                    try{
                        ...
                        if(Objects.equals(obj.get("code"), 0)){
                            ...
                            for(int i=0;i<len;i++){
                                ...
                                result.append("app名:").append(object.getString("name")).append(" ").append("app包名:").append(object.getString("packagename")).append(" ").append("app版本名:").append(object.getString("versionname")).append(" ").append("app版本号:").append(object.getInteger("versioncode")).append("应用商店id:").append(object.getInteger("id").toString()).append("\n");
                                String sha1=object.getString("sha1");
                                if (sha1!=null){
                                    result.append("sha1:").append(sha1).append("\n");
                                }else{
                                    result.append("sha1:没有sha1签名验证，此包名可以被改包安装\n");
                                }
                                result.append("应用商店下载链接:https://cloud.linspirer.com:883/download.php?email=").append(account).append("&appid=").append(object.getInteger("id").toString()).append("&swdid=").append(swdid).append("\n");
                            }
                            final String result1= result.toString();
                            textview_information.post(() -> {
                                if(!result1.equals("")){
        ...
        btn_getinfo.setOnClickListener(view -> {
